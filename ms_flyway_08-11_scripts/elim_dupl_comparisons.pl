...
open (PARSEOUTDUPLICATES, "Tony/Influenza_Genomics/OSU_Surveillance_Analysis/Blast/ALL_100/ALL_100_constgeneflow_events_PARSED.txt")||die "cannot open file"; # The file that was created above
open ($OUTPUT1, ">", "Tony/Influenza_Genomics/OSU_Surveillance_Analysis/Blast/ALL_100/ALL_100_constgeneflow_events_PARSED_dupl_removed.txt")||die "cannot open file";
...
while (<PARSEOUTDUPLICATES>){ # Enter into line by line to the PARSED file that contains all similarity events by segment, time, dist and number of constellation shares
	...
	$match1 = $record1[0]; # Assign the first isolate
	$match2 = $record1[1]; # Assign the second isolate
	
	if ($record1[4] == 0) { # If the time column is 0 then we are going to have to seperate out duplicates 
